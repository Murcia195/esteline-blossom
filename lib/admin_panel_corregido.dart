...
class _AdminPanelState extends State<AdminPanel> {
  ...
  // Suscribirse a cambios en tiempo real (polling inteligente)
  ...
  // Verificar cambios sin recargar siempre
  ...
  // Actualizar estado de compra - SIN admin_comment
      ...
      if (mounted) {
        setState(() {
          ...
          if (index != -1) {
            ...
            pendingPurchases[index]['updated_at'] = DateTime.now()
                .toIso8601String();
          }
        });
      }
              ...
              children: STATUS_OPTIONS.map((option) {
                ...
                return Padding(
                  ...
                  child: GestureDetector(
                    onTap: () {
                      _showConfirmDialog(purchase['id'], option);
                    },
                    child: Container(
                      padding: const EdgeInsets.symmetric(
                        horizontal: 12,
  ...
  // Dialog de confirmación simplificado
  void _showConfirmDialog(String purchaseId, String newStatus) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('¿Cambiar a $newStatus?'),
        titleTextStyle: const TextStyle(
          color: primaryGold,
          fontSize: 18,
          fontWeight: FontWeight.w700,
